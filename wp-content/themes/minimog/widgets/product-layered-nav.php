	...
	class Minimog_WP_Widget_Product_Layered_Nav extends Minimog_WC_Widget_Base {
		...
		protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
			...
			if ( $taxonomy !== $this->get_current_taxonomy() ) {
				...
				echo '<form method="get" action="' . esc_url( $form_action ) . '" class="minimog-wp-widget-product-layered-nav-form">';
				echo '<select data-placeholder="' . esc_attr( $any_label ) . '" class="minimog-wp-widget-product-layered-nav-dropdown dropdown_layered_nav_' . esc_attr( $taxonomy_filter_name ) . '"' . ( $multiple ? 'multiple="multiple"' : '' ) . '>';
				...
				if ( 'or' === $query_type ) {
					echo '<input type="hidden" name="query_type_' . esc_attr( $taxonomy_filter_name ) . '" value="or" class="filter-query-type" />';
				}

				echo '<input type="hidden" name="filter_' . esc_attr( $taxonomy_filter_name ) . '" value="' . esc_attr( implode( ',', $current_values ) ) . '" class="filter-name" />';
				echo '</form>';
			}
