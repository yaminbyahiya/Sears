...
if ( ! class_exists( 'Minimog_Woo' ) ) {
	class Minimog_Woo {
		...
		public function initialize() {
			...
			add_filter( 'woocommerce_variable_sale_price_html', [ $this, 'product_minmax_price_html' ], 999, 2 );
			add_filter( 'woocommerce_variable_price_html', [ $this, 'product_minmax_price_html' ], 999, 2 );
		...
		public function format_price_range( $price, $from, $to ) {
			$price = sprintf( _x( '%1$s <span class="wc-price-separator">&ndash;</span> %2$s', 'Price range: from-to', 'minimog' ), is_numeric( $from ) ? wc_price( $from ) : $from, is_numeric( $to ) ? wc_price( $to ) : $to );

			return $price;
		}

		public function trim_zeroes_from_price() {
			$enable = get_option( 'woocommerce_price_decimal_no_zeroes' );
		...
		public function get_product_image( $product, $size = 'full', $args = [] ) {
			...
			if ( ! empty( $placeholder_image ) && is_numeric( $placeholder_image ) ) {
				...
				$image = '<img src="' . $src . '" alt="' . esc_attr( $product->get_name() ) . '"/>';
			}
		...
		public function get_shop_categories_carousel_style() {
			$category_style = Minimog::setting( 'shop_sub_categories_carousel_style' );

			if ( is_product_category() ) {
				$category_style = Minimog::setting( 'product_category_sub_categories_carousel_style' );
			}
			...
			<div class="minimog-skeleton-card style-01">
				...
				<?php if ( ! empty( $loop_settings['show_variation'] ) ) : ?>
					<div class="minimog-skeleton-colors">
						<?php echo str_repeat( '<div class="minimog-skeleton-item minimog-skeleton-color"></div>', rand( 2, 4 ) ); ?>
					</div>
				<?php endif; ?>
			</div>
			<?php
		...
		public function get_product_highlight_filter_options() {
			return apply_filters( 'minimog/wp_widget/product_highlight_filter/options', [
				''             => esc_html__( 'All Products', 'minimog' ),
				'best_selling' => esc_html__( 'Best Seller', 'minimog' ),
				'new_arrivals' => esc_html__( 'New Arrivals', 'minimog' ),
				'on_sale'      => esc_html__( 'Sale', 'minimog' ),
				'featured'     => esc_html__( 'Hot Items', 'minimog' ),
			] );
		}
