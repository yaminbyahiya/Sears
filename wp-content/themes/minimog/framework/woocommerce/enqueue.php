...
class Enqueue {
	...
	public function frontend_scripts() {
		...
		/**
		 * Used edition version for better performance.
		 * Used same handle name to get localize script.
		 */
		wp_deregister_script( 'wc-checkout' );
		wp_register_script( 'wc-checkout', MINIMOG_THEME_URI . "/assets/js/woo/checkout{$min}.js", [
			'jquery',
			'woocommerce',
			'wc-country-select',
			'wc-address-i18n',
		...
		wp_register_script( 'minimog-wc-single', MINIMOG_THEME_URI . "/assets/js/woo/single{$min}.js", [
			'jquery',
		...
		if ( is_product() ) {
			...
			wp_enqueue_script( 'minimog-wc-single' );
			...
			wp_localize_script( 'minimog-wc-single', '$minimogProductSingle', $single_product_variables );
