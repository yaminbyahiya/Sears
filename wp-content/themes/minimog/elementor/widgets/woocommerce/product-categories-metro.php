...
class Widget_Product_Categories_Metro extends Base {
	...
	private function add_grid_section() {
		...
		$metro_layout_repeater->add_responsive_control( 'cat_info_horizontal_alignment', [
			...
			'default'              => '',
			'selectors'            => [
				'{{WRAPPER}} .minimog-product-categories.style-06 {{CURRENT_ITEM}} .category-info' => 'display: flex; justify-content: {{VALUE}}',
			],
			'conditions'           => [
				'terms' => [
					[
		...
		$metro_layout_repeater->add_responsive_control( 'cat_info_vertical_alignment', [
			...
			'default'              => '',
			'selectors'            => [
				'{{WRAPPER}} .minimog-product-categories.style-06 {{CURRENT_ITEM}} .category-info' => 'display: flex; align-items: {{VALUE}}',
			],
			'conditions'           => [
				'terms' => [
					[
	...
	protected function render() {
		...
		if ( '01' === $settings['style'] ) {
			$style = '05';
		} elseif ( '02' === $settings['style'] ) {
			$style = '02';
		} elseif ( '03' === $settings['style'] ) {
			$style = '06';
		}
	...
	private function print_category_count( $category, $text = true ) {
		...
		<div class="category-count">
			...
			<?php if ( $text ) : ?>
				<span
					class="cat-count-text"><?php echo esc_html( _n( 'item', 'items', $category->count, 'minimog' ) ); ?></span>
			<?php endif; ?>
		</div>
		<?php
	}
