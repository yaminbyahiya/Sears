	...
	function( $ ) {
		...
		var MinimogTabPanel = function( $el, options ) {
			...
			this.init = function() {
				...
				if ( 'dropdown' === plugin.settings.navType ) {
					...
					$el.on( 'click', '.tab-title', function( evt ) {
						...
						$thisTab.siblings().removeClass( plugin.ACTIVE_CLASS );
						$thisTab.addClass( plugin.ACTIVE_CLASS );
			...
			this.switchTab = function( index ) {
				...
				$panel.removeClass( plugin.ACTIVE_CLASS );

				var $activeTabContent = $panel.filter( '[data-tab="' + index + '"]' ).addClass( plugin.ACTIVE_CLASS );
